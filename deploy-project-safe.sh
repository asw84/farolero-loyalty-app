...
echo "🚀 Деплой проекта Farolero с сохранением ВСЕХ настроек"
echo "====================================================="
...
echo "🏗️  7. Полная пересборка контейнеров с новой версией Node.js 20..."
docker-compose build --no-cache --build-arg BUILD_VERSION=$BUILD_VERSION

echo "🚀 8. Запускаем контейнеры с очисткой кеша..."
docker-compose up -d --force-recreate

# Очищаем кеш браузера на стороне клиента (инструкция)
echo ""
echo "🧹 9. Инструкция по очистке кеша браузера:"
echo "   После деплоя очистите кеш браузера или нажмите Ctrl+F5 / Cmd+Shift+R"
